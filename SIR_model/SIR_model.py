...
class Simulator:
    def __init__(self, n_agents=10, b_shape=(10, 10), sick_agents=1, beta=0.75, gamma=0.75, death_risk=0.1,
                 disease_spread_distance=1, moving_range=1,  time_steps=5, nr=1):
    ...
    def agents_spread_disease(self):
        def agent_spreads_disease(agent):
            if agent.is_sick:
                cx, cy = agent.pos
                for agent in self.agents:
                    x, y = agent.pos
                    if agent.susceptible and ((cx-x)**2+(cy-y)**2) <= self.disease_spread_distance**2 \
                            and rd.random() < self.beta:
        ...
        with ThreadPoolExecutor() as executor:
            for agent in self.agents:
                executor.submit(agent_spreads_disease(agent))


if __name__ == "__main__":
    from defs import sim
    try:
        N_SIMULATIONS = int(os.environ['NUMBER_OF_PROCESSORS'])
        ...
        try:
            ...
            fig = plot.get_figure()
            fig.savefig('Disease_spread.png')
        except Exception as e:
            print(e)
        else:
            try:
                from PIL import Image; Image.open('Disease_spread.png').show()
                os.remove('Disease_spread.png')
            except Exception as e:
                print(e)
    finally:
        for file in sims:
